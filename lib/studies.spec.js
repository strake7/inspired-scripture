...
describe('getStudyBySlug', () => {
  ...
  it('trims trailing whitespace from title', () => {
    fs.readFileSync.mockImplementation((filePath, encoding) => {
      if (filePath.includes('psalm-23.html')) {
        return '<h1>Psalm 23 Study</h1>\nBackground: A psalm of David about the Lord as shepherd.'
      }
      if (filePath.includes('study_metadata.yml')) {
        return '{}'
      }
      return ''
    })
  ...
  it('extracts description skipping Introduction period prefix', () => {
    fs.readFileSync.mockImplementation((filePath, encoding) => {
      if (filePath.includes('job-1.html')) {
  ...
  it('parses book and chapter from three-word book format', () => {
    const study = getStudyBySlug('song-of-solomon-4', [])

    expect(study.book).toBe('Song of Solomon')
    expect(study.chapter).toBe(4)
    expect(study.suffix).toBe('')
    expect(study.chapterLabel).toBe('4')
  })
  ...
  beforeEach(() => {
    ...
    fs.readFileSync.mockImplementation((path, encoding) => {
      if (path.includes('genesis-1.html')) {
        return '<h1>Genesis 1</h1>\nIntroduction: A study about the creation account in Genesis chapter one.'
      }
      if (path.includes('genesis-2.html')) {
        return '<h1>Genesis 2</h1>\nIntroduction: A study about the creation account in Genesis chapter two.'
      }
      if (path.includes('psalm-23.html')) {
        return '<h1>Psalm 23</h1>\nIntroduction: A study about the Lord as shepherd in Psalm twenty three.'
      }
      if (path.includes('study_metadata.yml')) {
        return JSON.stringify(mockMetadata)
      }
